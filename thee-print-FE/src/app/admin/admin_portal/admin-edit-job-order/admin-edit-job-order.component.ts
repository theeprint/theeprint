...
@Component({
  selector: 'app-admin-edit-job-order',
  templateUrl: './admin-edit-job-order.component.html',
  styleUrls: ['./admin-edit-job-order.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class AdminEditJobOrderComponent implements OnInit {
  today = new Date();
  user_id:any;
  partner_details:any
  ...
  editMachinePrint: any;

  openEditProductModal(index){
    console.log(this.job_order_item[index])
    ...
    // alert(this.job_order_item[index].machine_print_id);
  ...
  addJob(){
    if (this.addItemForm.valid && this.addJobForm.valid) {
      ...
      console.log('asa',this.addJobForm.controls.job_order_due_date.value);
      ...
      this.userService.updateJob(params).then(response => {
        ...
        this.router.navigate(['/admin/admin-partners-quotation/'+job_id]);
      })
    }else{
  ...
  setPartners(partner_company): void {
  ...
  editJobOrderProduct(){
    ...
    if (this.editJobForm.valid) {
      ...
      this.job_order_item.push(params);
      console.log('edit', this.job_order_item)
      this.ngxSmartModalService.close('editProduct');
  ...
  editMediaChange(productMedia): void {
  ...
  getJobOrders(job_order_request_id){
    this.userService.getJobOderdetails(job_order_request_id).then(response => {
      ...
      console.log('qwrty1', this.job_order_details[0].job_order_due_date);
  ...
  saveAsDraft(){  
    ...
    this.userService.updateJobOrder(this.job_order_request_id, params).then(response => {
      ...
      setTimeout(()=>{    
        ...
        setTimeout(()=>{ 
        window.location.href = "/admin/job-order-list/draft";
        }, 500);
      }else{
        this.spinner.hide(); 
      }
