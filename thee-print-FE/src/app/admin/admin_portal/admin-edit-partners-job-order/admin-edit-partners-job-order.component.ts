...
@Component({
  selector: 'app-admin-edit-partners-job-order',
  templateUrl: './admin-edit-partners-job-order.component.html',
  styleUrls: ['./admin-edit-partners-job-order.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class AdminEditPartnersJobOrderComponent implements OnInit {
  user_id:any;
  partner_details:any
  ...
  addJob(){
    if (this.addItemForm.valid && this.addJobForm.valid) {
      ...
      this.userService.updateJob(params).then(response => {
        ...
        // this.userService.sendEmailPriceCheck(this.partner_company);
        this.router.navigate(['/admin/job-order-details/'+job_id]);
      })
    }else{
